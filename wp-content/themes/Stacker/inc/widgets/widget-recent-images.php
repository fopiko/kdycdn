...
class themefurnace_Widget_Recent_Images extends WP_Widget
{
	...
	function form( $instance )
	{
		...
		<div class="widget-controls columns-1">
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'order' ); ?>"><?php _e( 'Order:', 'themefurnace' ); ?></label>
				<select class="widefat" id="<?php echo $this->get_field_id( 'order' ); ?>"
						name="<?php echo $this->get_field_name( 'order' ); ?>">
					<?php foreach ( $order as $option_value => $option_label ) { ?>
						<option
							value="<?php echo esc_attr( $option_value ); ?>" <?php selected( $instance['order'], $option_value ); ?>><?php echo esc_html( $option_label ); ?></option>
					<?php } ?>
				</select>
			</p>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e( 'Order By:', 'themefurnace' ); ?></label>
				<select class="widefat" id="<?php echo $this->get_field_id( 'orderby' ); ?>"
						name="<?php echo $this->get_field_name( 'orderby' ); ?>">
					<?php foreach ( $orderby as $option_value => $option_label ) { ?>
						<option
							value="<?php echo esc_attr( $option_value ); ?>" <?php selected( $instance['orderby'], $option_value ); ?>><?php echo esc_html( $option_label ); ?></option>
					<?php } ?>
				</select>
			</p>
