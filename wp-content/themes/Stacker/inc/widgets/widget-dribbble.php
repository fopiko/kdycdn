...
class themefurnace_Widget_Dribbble_Shots extends WP_Widget
{
	...
	/**
	 * Wrapper function for the filter
	 */
	function dribbleCacheTime()
	{
		return 1800;
	}
	...
	function widget( $args, $instance )
	{
		...
		$args['show_shot_title'] = esc_html( $instance['show_shot_title'] );
		$args['show_shot_date'] = esc_html( $instance['show_shot_date'] );
		...
		if ( function_exists( 'fetch_feed' ) ) :
			...
			add_filter( 'wp_feed_cache_transient_lifetime', 'dribbleCacheTime' );
			if ( !is_wp_error( $rss ) ) :
	...
	function form( $instance )
	{
		...
		<div class="widget-controls">
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'themefurnace' ); ?></label>
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'username' ) ); ?>"><?php _e( 'Username:', 'themefurnace' ); ?></label>
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'username' ) ); ?>"
					   name="<?php echo esc_attr( $this->get_field_name( 'username' ) ); ?>"
					   value="<?php echo esc_attr( $instance['username'] ); ?>" type="text"/>
			</p>

			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php _e( 'Limit:', 'themefurnace' ); ?></label>
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"
					   name="<?php echo esc_attr( $this->get_field_name( 'limit' ) ); ?>"
					   value="<?php echo esc_attr( $instance['limit'] ); ?>" type="number"/>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_shot_title' ) ); ?>">
					<input class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_shot_title' ) ); ?>"
						   name="<?php echo esc_attr( $this->get_field_name( 'show_shot_title' ) ); ?>"
						   type="checkbox" <?php checked( $instance['show_shot_title'], true ); ?> /> <?php _e( 'Show shot title?', 'themefurnace' ); ?>
				</label>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_shot_date' ) ); ?>">
					<input class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_shot_date' ) ); ?>"
						   name="<?php echo esc_attr( $this->get_field_name( 'show_shot_date' ) ); ?>"
						   type="checkbox" <?php checked( $instance['show_shot_date'], true ); ?> /> <?php _e( 'Show shot date?', 'themefurnace' ); ?>
				</label>
			</p>
		</div><!-- .widget-controls -->
